...
func (self *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	process := func(worker int) {
		for index := range tasks {
			...
			if err := WritePublicReceipts(self.chainDb, receipts); err != nil {
...
func (self *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	...
	for i, block := range chain {
		...
		if err := WriteBlockReceipts(self.chainDb, block.Hash(), block.NumberU64(), publicReceipts); err != nil {
			return i, err
		}
		...
		case CanonStatTy:
			...
			// store the public receipts
			if err := WritePublicReceipts(self.chainDb, publicReceipts); err != nil {
				return i, err
			}
			// store the private receipts
